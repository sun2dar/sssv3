...
import { getEntities } from './t-plywood.reducer';
import { ITPlywood } from 'app/shared/model/t-plywood.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';

export interface ITPlywoodProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export type ITPlywoodState = IPaginationBaseState;

export class TPlywood extends React.Component<ITPlywoodProps, ITPlywoodState> {
  state: ITPlywoodState = {
  ...
  render() {
    const { tPlywoodList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="t-plywood-heading">
          T Plywoods
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp; Create new T Plywood
          </Link>
        </h2>
        <div className="table-responsive">
          <Table responsive>
            ...
            <tbody>
              {tPlywoodList.map((tPlywood, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${tPlywood.id}`} color="link" size="sm">
                      {tPlywood.id}
                    </Button>
                  </td>
                  <td>{tPlywood.qty}</td>
                  <td>{tPlywood.volume}</td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${tPlywood.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${tPlywood.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${tPlywood.id}/delete`} color="danger" size="sm">
...
const mapStateToProps = ({ tPlywood }: IRootState) => ({
  tPlywoodList: tPlywood.entities,
  totalItems: tPlywood.totalItems
});
