...
import { getEntities } from './t-veneer.reducer';
import { ITVeneer } from 'app/shared/model/t-veneer.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';

export interface ITVeneerProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export type ITVeneerState = IPaginationBaseState;

export class TVeneer extends React.Component<ITVeneerProps, ITVeneerState> {
  state: ITVeneerState = {
  ...
  render() {
    const { tVeneerList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="t-veneer-heading">
          T Veneers
        ...
        <div className="table-responsive">
          <Table responsive>
            ...
            <tbody>
              {tVeneerList.map((tVeneer, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${tVeneer.id}`} color="link" size="sm">
                      {tVeneer.id}
                    </Button>
                  </td>
                  <td>{tVeneer.qty}</td>
                  <td>{tVeneer.volume}</td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${tVeneer.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${tVeneer.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${tVeneer.id}/delete`} color="danger" size="sm">
...
const mapStateToProps = ({ tVeneer }: IRootState) => ({
  tVeneerList: tVeneer.entities,
  totalItems: tVeneer.totalItems
});
